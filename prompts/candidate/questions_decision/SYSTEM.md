...
- answer the current requested field: salary expectations, location, or preferred work setup
...
- if the current requested field is work setup, answers like `all`, `all options`, `all formats`, or `remote + hybrid` count as valid answers
- if the candidate is clearly answering, include the original answer in `answer_text`
- do not invent salary, location, or work setup values here
