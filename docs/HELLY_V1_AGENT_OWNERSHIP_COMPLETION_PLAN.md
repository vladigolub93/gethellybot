...
23. Remove deterministic intent classification from `VERIFICATION_PENDING`. `Completed`
...
34. Remove deterministic intent classification from `OPEN`. `Completed`
...
48. Keep only action-oriented execution methods in domain services. `Completed`
