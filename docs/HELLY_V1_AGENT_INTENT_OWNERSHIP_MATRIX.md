...
| `VERIFICATION_PENDING` | `agent-first (mostly completed)` | `agent-first` | text meaning is stage-agent owned; raw `video` remains a Telegram media event by design | [candidate.py](/Users/vladigolub/Desktop/gethellybot/src/graph/stages/candidate.py) |
...
| `INTERVIEW_IN_PROGRESS` | `agent-first (mostly completed)` | `agent-first` | live validation still needed; backend now executes explicit `answer_current_question`, while voice/video media handling remains a backend transport path by design | [candidate.py](/Users/vladigolub/Desktop/gethellybot/src/graph/stages/candidate.py), [interview/service.py](/Users/vladigolub/Desktop/gethellybot/src/interview/service.py) |
