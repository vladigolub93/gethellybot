...
- `TelegramUpdateService` no longer depends directly on `BotControllerService`; generic unsupported-input recovery now resolves current stage context through graph/messaging-native transport logic
...
- entry-stage routing and role-flow dispatch are now also extracted into `_apply_entry_flow(...)`, `_precompute_role_stage_results(...)`, and `_apply_role_flows(...)`, reducing orchestration depth inside `_apply_identity_flow`
...
- Phase 11 live inspection/reporting now also exposes the latest raw Telegram message and latest state transition for faster debugging of stalled smoke runs
