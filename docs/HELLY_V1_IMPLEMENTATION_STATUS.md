...
- candidate, vacancy, interview, and evaluation services now use explicit `execute_*` action methods as the graph-owned runtime path; stage meaning is no longer delegated to raw-text compatibility handlers in domain services.
...
- Telegram webhook processing now has an immediate-delivery fallback for notifications created by the current inbound update, so the user can still receive stage replies even if the background notification worker is temporarily stalled.
- Telegram webhook processing now also has an immediate best-effort fallback for newly created file storage and first-pass candidate/vacancy extraction jobs, so initial CV/JD intake no longer blocks entirely on a healthy background worker.
