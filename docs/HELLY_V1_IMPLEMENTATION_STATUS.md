...
- candidate `CV_PENDING`, `SUMMARY_REVIEW`, and `QUESTIONS_PENDING` help handling have also started migrating through graph-driven paths
...
- `Partial`: `CV_PENDING`, `SUMMARY_REVIEW`, and `QUESTIONS_PENDING` help execution have started migrating to bounded LangGraph stage agents, while the rest of candidate onboarding still runs through the legacy state-aware controller path
