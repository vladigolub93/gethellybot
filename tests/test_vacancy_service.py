...
def test_clarification_partial_assessment_answer_asks_targeted_live_coding_follow_up() -> None:
    service = VacancyService(FakeSession())
    fake_repo = FakeVacanciesRepository()
    fake_state = FakeStateService()
    service.repo = fake_repo
    ...
    result = service.handle_clarification_answer(
        ...
        raw_message_id=uuid4(),
        content_type="text",
        text="тестовая таска есть",
    )

    assert result is not None
    assert result.status == "follow_up"
    assert vacancy.has_take_home_task is True
    assert getattr(vacancy, "has_live_coding", None) is None
    assert vacancy.questions_context_json["current_question_key"] == "assessment"
    assert "live coding" in result.notification_text.lower()
...
def test_clarification_accepts_free_take_home_answer_and_moves_to_hiring_stages() -> None:
    service = VacancyService(FakeSession())
    fake_repo = FakeVacanciesRepository()
    fake_state = FakeStateService()
    service.repo = fake_repo
    ...
    assert result is not None
    assert result.status == "next_question"
    assert vacancy.take_home_paid is False
    assert vacancy.questions_context_json["current_question_key"] == "hiring_stages"
