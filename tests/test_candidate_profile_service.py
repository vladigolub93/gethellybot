...
def test_questions_accept_higher_than_average_english_and_move_to_domains() -> None:
    service = CandidateProfileService(FakeSession())
    fake_repo = FakeCandidateProfilesRepository()
    fake_state = FakeStateService()
    service.repo = fake_repo
    ...
    assert profile.questions_context_json["current_question_key"] == "preferred_domains"


def test_questions_accept_no_domain_preference_and_move_to_assessment() -> None:
    service = CandidateProfileService(FakeSession())
    fake_repo = FakeCandidateProfilesRepository()
    fake_state = FakeStateService()
    service.repo = fake_repo
    ...
    assert result is not None
    assert result.status == "next_question"
    assert profile.preferred_domains_json == ["any"]
    assert profile.questions_context_json["current_question_key"] == "assessment_preferences"
...
def test_questions_accept_take_home_only_phrase_and_complete() -> None:
    service = CandidateProfileService(FakeSession())
    fake_repo = FakeCandidateProfilesRepository()
    fake_state = FakeStateService()
    service.repo = fake_repo
    ...
    result = service.handle_questions_answer(
        ...
        raw_message_id=uuid4(),
        content_type="text",
        text="я же сказал что только тестовая таска ок",
    )
